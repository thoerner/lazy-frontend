...
import {
  ...
  ClaimMessage,
  ConnectMessage,
} from "../components/ButtMessages.jsx";
...
const MyButts = ({
  ...
  const shouldRequestSignMessage = () => {
    return (
      !authenticated && myButts.length > 0 && token !== "" && !signingMessage
    );
  };
  ...
  useEffect(() => {
    ...
    const fetchCubs = async (address) => {
      const cubs = [];
      try {
        const data = await getCubs(address);
        for (let i = 0; i < data.length; i++) {
          cubs.push({ id: data[i] });
        }
        if (data) {
        ...
        if (ENV === "dev") {
          setMyCubs([{ id: "20407" }, { id: "30407" }]);
          console.log("setting cubs to test values");
        } else {
          console.log(`problem fetching cubs for ${address}`);
        }
      }
    };
    if (isConnected) {
  ...
  const renderContent = () => {
    ...
    if (isConnected && !authenticated && myButts.length > 0)
      return <SignMessage handleSignClick={handleSignClick} />;
  ...
  return (
    <div className="myButts">
      ...
      {!isMobile ? (
        <>
          <p>
            Welcome to your personal collection of Lazy Butts. Here, you can
            view, manage, and interact with all the Lazy Butts you've claimed or
            acquired. Each unique asset is a testament to our vibrant and
            growing community. Dive in, explore your collection, and amplify the
            roar of the Pride.
          </p>
          <p>
            It's important to note: While Lazy Butts owners enjoy exclusive
            access to the high-resolution Lazy Butt image and other potential
            assets, the Full Body artwork is a special privilege. To unlock the
            complete Full Body artwork, you must own BOTH the Lazy Butt AND the
            corresponding Lazy Lion. This pairing ensures a comprehensive and
            unique representation of your chosen character in the Pride.
          </p>
        </>
      ) : (
        <>
          <p>
            Welcome to your Lazy Butts collection! Browse and manage your unique
            assets, and join our thriving community. Dive into your collection
            and feel the Pride's roar.
          </p>
          <p>
            Remember, only by owning both a Lazy Butt and its matching Lazy Lion
            can you access the full artwork. This combo fully showcases your
            Pride character.
          </p>
        </>
      )}
      <Disclaimer />
      <div className="myButtsContent">{renderContent()}</div>
      <Footer />
    </div>
  );
};
