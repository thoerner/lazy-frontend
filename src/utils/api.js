...
export const getMetadata = async (buttId) => {
  const res = await fetch(
    `${import.meta.env.VITE_API_URL}/api/metadata/${buttId}.json`
  );
...
export const getToken = async (address) => {
  const res = await fetch(
    `${import.meta.env.VITE_API_URL}/api/auth/token?address=${address}`
  );
...
export const getProof = async (address) => {
  const res = await fetch(
    `${import.meta.env.VITE_API_URL}/api/auth/proof?address=${address}`
  );
